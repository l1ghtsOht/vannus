...
export default function SurvivorCard({ survivor, index, isExpanded, onToggle }) {
  ...
  return (
    <motion.div
      ...
      className={`glass overflow-hidden group relative cursor-pointer ${isPinned ? 'bg-indigo-500/5' : ''}`}
    ...
    >
        ...
        <div className="flex-1 p-3 pl-4">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <span className="text-white/30 text-[10px] transition-transform duration-200" style={{ transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)' }}>{'\u25B8'}</span>
      ...
      {/* Detail Drawer */}
      <AnimatePresence>
        {isExpanded && <ToolDetailDrawer tool={survivor} />}
      </AnimatePresence>
    </motion.div>
  );
}
